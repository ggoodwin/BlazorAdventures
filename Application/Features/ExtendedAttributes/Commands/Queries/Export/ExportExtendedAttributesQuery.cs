...
using Application.Specifications.ExtendedAttribute;
...
namespace Application.Features.ExtendedAttributes.Commands.Queries.Export
{
            ...
            where TId : IEquatable<TId>
    {
        ...
        public async Task<Result<string>> Handle(ExportExtendedAttributesQuery<TId, TEntityId, TEntity, TExtendedAttribute> request, CancellationToken cancellationToken)
        {
            ...
            if (request.IncludeEntity)
            {
                ...
                mappers.Add("EntityLastModifiedBy", item => item.Entity.LastModifiedBy);
                mappers.Add("EntityLastModifiedOn (Local)", item => item.Entity.LastModifiedOn?.ToString("G", CultureInfo.CurrentCulture));
                mappers.Add("EntityLastModifiedOn (UTC)", item => item.Entity.LastModifiedOn != null ? DateTime.SpecifyKind((DateTime)item.Entity.LastModifiedOn, DateTimeKind.Utc).ToLocalTime().ToString("G", CultureInfo.CurrentCulture) : string.Empty);
            }

            var data = await _excelService.ExportAsync(extendedAttributes, mappers: mappers,
                sheetName: $"{typeof(TEntity).Name} Extended Attributes");
