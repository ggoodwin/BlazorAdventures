...
using Client.Infrastructure.Managers.Misc.DocumentType;
...
namespace Client.Pages.Misc
{
    public partial class DocumentTypes
    {
        ...
        private async Task ExportToExcel()
        {
            ...
            if (response.Succeeded)
            {
                ...
                _snackBar.Add(string.IsNullOrWhiteSpace(_searchString)
                    ? "Document Types exported"
                    : "Filtered Document Types exported", Severity.Success);
            }
            else
            {
                foreach (var message in response.Messages)
        ...
        private async Task InvokeModal(int id = 0)
        {
            ...
            var options = new DialogOptions { CloseButton = true, MaxWidth = MaxWidth.Small, FullWidth = true, DisableBackdropClick = true };
            var dialog = _dialogService.Show<AddEditDocumentTypeModal>(id == 0 ? "Create" : "Edit", parameters, options);
            var result = await dialog.Result;
            if (!result.Cancelled)
            {
                await Reset();
