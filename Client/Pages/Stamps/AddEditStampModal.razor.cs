using Application.Features.Stamps.Commands.AddEdit;
using Blazored.FluentValidation;
using Client.Infrastructure.Managers.Stamp;
using Microsoft.AspNetCore.Components;
using MudBlazor;

namespace Client.Pages.Stamps
{
    public partial class AddEditStampModal
    {
        [Inject] private IStampManager StampManager { get; set; }

        [Parameter] public AddEditStampCommand AddEditStampModel { get; set; } = new();
        [CascadingParameter] private MudDialogInstance MudDialog { get; set; }
        //[CascadingParameter] private HubConnection HubConnection { get; set; }

        private FluentValidationValidator _fluentValidationValidator;
        private bool Validated => _fluentValidationValidator.Validate(options => { options.IncludeAllRuleSets(); });

        public void Cancel()
        {
            MudDialog.Cancel();
        }

        private async Task SaveAsync()
        {
            var response = await StampManager.SaveAsync(AddEditStampModel);
            if (response.Succeeded)
            {
                _snackBar.Add(response.Messages[0], Severity.Success);
                MudDialog.Close();
            }
            else
            {
                foreach (var message in response.Messages)
                {
                    _snackBar.Add(message, Severity.Error);
                }
            }
            //await HubConnection.SendAsync(ApplicationConstants.SignalR.SendUpdateDashboard);
        }

        protected override async Task OnInitializedAsync()
        {
            await LoadDataAsync();
            //HubConnection = HubConnection.TryInitialize(_navigationManager, _localStorage);
            //if (HubConnection.State == HubConnectionState.Disconnected)
            //{
            //    await HubConnection.StartAsync();
            //}
        }

        private async Task LoadDataAsync()
        {
            await Task.CompletedTask;
        }
    }
}
